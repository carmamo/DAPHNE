...
{
  /* Clear CAN bus wakeup interrupt */
  MCP2515_BitModify(SPI_CAN, MCP2515_CANINTF, 0x40, 0x00);
  
  /* Enable CAN bus activity wakeup */
  MCP2515_BitModify(SPI_CAN, MCP2515_CANINTE, 0x40, 0x40);
...
uint8_t CANSPI_isBussOff(SPI_HandleTypeDef *SPI_CAN)
{
  ...
  errorStatus.error_flag_reg = MCP2515_ReadByte(SPI_CAN, MCP2515_EFLG);
